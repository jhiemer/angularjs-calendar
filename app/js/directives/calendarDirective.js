define(['moment', 'angular'], function (moment, angular) {
	...
	var calendarDirectiveModule,
		buildArray, sortEventByStartAndDuration, filterEventDuringDateRangeFactory,
		isWholeDayEvent, colspanify, slotFactory, timeout, getStartOfWeek, hoverSelektor, uniqueFilter,
		DEBUG = false,
	 ...
	 */
	sortEventByStartAndDuration = function sortEventByStartAndDuration(a, b) {
	 ...
	 */
	filterEventDuringDateRangeFactory = function (start, end) {
		return function (event) {
			...
			return event.end.diff(start) > 0 && event.start.diff(end) < 0;
	 ...
	 *
	 */
	colspanify = function colspanify(days, events) {
		//TODO - das ist kein emptry array sondern undefined!
		...
		events.map(function (event) {
			...
			//TODO - zwei Zuweisungen in einer Zeile, Zuweisung an current wierd nie benutzt
			current = slots[slot] = (slot in result ? slots[slot] : 0);

			result[slot] = result[slot] || [];

			start = Math.min(length, Math.max(0, eventStart.diff(firstDay, 'days')));
	...
	timeout = function (scope, fn, timeout) {
	...
	//TODO - getStartOfWeek wird nie benutzt
	//TODO - Parameter date der im JSDoc erwähnt wird, gibt es gar nicht
	/**
	 ...
	 *  @return date
	 **/
	getStartOfWeek = function startOfWeek(day) {
	...
	//TODO - jsDOC passt überhaupt nicht zur Methode
	/**
	 ...
	 *  @return element
	 **/
	hoverSelektor = function (element, selector, klass) {
		return element.hover(function () {
				angular.element(selector).addClass(klass);
			}
			, function () {
				angular.element(selector).removeClass(klass);
			}
		);
	};
	 ...
	 **/
	uniqueFilter = function (item, index, list) {
		return list.indexOf(item, index + 1) < 0;
	};

	calendarDirectiveModule = angular.module('directive.calendar', []);

	calendarDirectiveModule.factory('eventEmitter', function () {
		var events = {},
			seperator = '::',
			getEvent,
			Event,
		...
		//TODO - unsubscribe und clear werden nicht benutzt
		eventEmitter = {
	 ...
	 */
	calendarDirectiveModule.factory('eventService', ['eventEmitter', function (eventEmitter) {
		var rawDayEvents = [],
			dayEvents = {},
			wholeDayEvents = [],
		...
		//TODO - inkonsistente return Statements: einmal true und einmal nichts (=undefined)
		...
		eventExists = function (item, list) {
			var result = false;
		...
		EventEmitter = eventEmitter.create('eventService');
		...
		eventService = {
			storeEvents:function (events) {
				...
				dayEvents = {};
				events.map(function (event) {
					addItemToList(event, isWholeDayEvent(event) ? wholeDayEvents : rawDayEvents);
				});
				[rawDayEvents, wholeDayEvents].map(eventService.sortEvents);
				[rawDayEvents, wholeDayEvents].map(function (list) {
					list.map(slotter);
				});
	...
	calendarDirectiveModule.directive('wholeDayEvent', [function () {
		return {
			restrict:'E', replace:true, template:'<div name="bs-calendar-event-id-{{event.id}}" class="bs-calendar-whole-day-event-container evt-{{event.id}}">'
	...
	calendarDirectiveModule.directive('hourViewNow', function () {
		return {
			restrict:'E', replace:true, template:'<div>' +
				'<div class="bs-tg-now"></div>' +
				'</div>', link:function (scope, iElement) {
				...
				nowDiv.css({
					display:display, position:'absolute', top:start_seconds * height
				});
			}
		};
	});
	 ...
	 *
	 */
	calendarDirectiveModule.directive('hourViewEvent', [function () {
		return {
			restrict:'E', replace:true, template:'<div name="bs-calendar-event-id-{{event.id}}" class="bs-calendar-event-container">'
			...
			link:function postLink(scope, iElement) {
				var day = scope.$parent.$parent.day
					...
					height = CALENDAR_HEIGHT / SECONDS_OF_A_DAY;

				iElement.css({
					left:((scope.event.slot - 1) * SLOT_WIDTH) + '%', top:start_seconds * height, height:end_seconds * height
				});
	 ...
	 */
	calendarDirectiveModule.directive('hourViewEventContainer', function () {
		return {
			restrict:'E',
			replace:true,
	...
	calendarDirectiveModule.directive('wholeDayView', ['eventService', function (eventService) {
		return {
	 ...
	 *
	 */
	calendarDirectiveModule.directive('calendarDayView', ['eventService', function (eventService) {
		return {
			...
			link:function postLink(scope) {
				...
				var update = timeout(scope, function () {
					...
					scope.days = buildArray(numberOfDays).map(function (index) {
						...
						day.events = eventService.getDayEvents(day);
						return day;
					});
				});
	...
	calendarDirectiveModule.factory('daysOfWeek', function () {
		return function (day) {
			var base = day.clone(),
				result = [];
	 ...
	 *
	 */
	calendarDirectiveModule.directive('calendarMonthView', ['daysOfWeek', function (daysOfWeek) {
		return {
	 ...
	 */
	calendarDirectiveModule.directive('calendar', function () {
		return {
			restrict:'E',
			replace:true,
	...
	return calendarDirectiveModule;
});
