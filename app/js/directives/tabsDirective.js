define(['angular'], function (angular) {
	...
	var tabsDirectiveModule = angular.module('directive.tabs', []);

	tabsDirectiveModule.directive('tabs',function () {
		return {
			restrict:'E',
			transclude:true,
		...
		return {
			...
			restrict:'E',
			transclude:true,
			template:'<div class="tab-pane" style="height: {{paneHeight}}px" ng-class="{active: selected}" ng-transclude>' +
				'</div>',
			replace:true,
			scope:{
				paneTitle:'@',
			...
			link:function (scope, element, attrs, tabsCtrl) {
				...
				//TODO - ohne strict Modus wird hier eine globale Variable angelegt, mit strict Modus gibts einen Fehler, Original-Code erstmal auskommentiert
				//$scope = scope.$parent;

				parentScope = scope.$parent;

				scope.prev = attrs.panePrev ? function () {
					parentScope.$eval(attrs.panePrev);
				} : angular.noop;
				scope.next = attrs.paneNext ? function () {
					parentScope.$eval(attrs.paneNext);
				} : angular.noop;
